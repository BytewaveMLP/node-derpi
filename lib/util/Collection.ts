...
export abstract class Collection<T1, T2> implements AsyncIterable<T2> {
	 ...
	 * @memberof Collection
	 */
	public async find(fn: (value: T2) => boolean): Promise<T2 | null> {
	 ...
	 * @memberof Collection
	 */
	public async map<T>(fn: (value: T2) => T): Promise<T[]> {
		let arr: T[] = [];
	 ...
	 * @memberof Collection
	 */
	public async some(fn: (value: T2) => boolean): Promise<boolean> {
		for await (const value of this) {
	 ...
	 * @memberof Collection
	 */
	public async every(fn: (value: T2) => boolean): Promise<boolean> {
	 ...
	 * @memberof Collection
	 */
	public async reduce<T>(fn: (acc: T, value: T2) => T, initialAcc: T): Promise<T> {
		let acc = initialAcc;
	 ...
	 * @memberof Collection
	 */
	public async tap(fn: (value: T2) => any): Promise<Collection<T1, T2>> {
		for await (const value of this) {
			fn(value);
		}
